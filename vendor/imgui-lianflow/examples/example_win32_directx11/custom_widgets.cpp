...
#include <string>
	...
	bool Keybind(const char* label, int* key, int* mode)
	{
		...
		it_anim->second.icon = ImLerp(it_anim->second.icon, g.ActiveId == id ? c::main_color : hovered ? c::text::label::hovered : c::text::label::default, GetAnimSpeed());
	...
	bool ChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool cap, ImGuiWindowFlags flags)
	{
		...
		GetWindowDrawList()->AddText(parent_window->DC.CursorPos + ImVec2(GetStyle().WindowPadding.x, 25 - CalcTextSize(name).y / 2), utils::GetColorWithAlpha(c::label::default, ImGui::GetStyle().Alpha), name, label_end);
	...
	bool Button(const char* label, const ImVec2& size_arg)
	{
		...
		it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? c::label::active : c::label::default, g.IO.DeltaTime * 6.f);
	...
	bool Tab(const char* label, int* v, int number)
	{
		...
		it_anim->second.text_col = ImLerp(it_anim->second.text_col, *v == number ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default, GetAnimSpeed());
	...
	bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
	{
		...
		ImRotateStart();
		GetWindowDrawList()->AddText(ImVec2(bb.Max.x - 24, bb_box.GetCenter().y - CalcTextSize(ICON_DOWN_SMALL_LINE).y / 2), c::label::default, ICON_DOWN_SMALL_LINE);
	...
	bool ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
	{
		...
		if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
		{
			...
			it_anim->second.icon = ImLerp(it_anim->second.icon, ImGui::IsPopupOpen("picker") ? c::main_color : IsMouseHoveringRect(rect.Min, rect.Max, true) ? c::text::label::hovered : c::text::label::default, GetAnimSpeed());
	...
	bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		st.text_color = ImLerp(st.text_color, held ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::default, speed);
