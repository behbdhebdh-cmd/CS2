...
namespace PhysExtractor.src
{
    class Program
    {
        static string? g_mapFilter = null;
        ...
        static void Main(string[] args)
        {
            ...
            {
                string a = args[i];
                if (a == "--all") mapChoice = 1;
                else if (a == "--official") mapChoice = 2;
                else if (a == "--workshop") mapChoice = 3;
                else if (a == "--both") formatChoice = 1;
                else if (a == "--tri") formatChoice = 2;
                else if (a == "--vphys") formatChoice = 3;
                else if (a == "--nopause") nopause = true;
                else if (a == "--out" && i + 1 < args.Length) outOverride = args[++i];
                else if (a == "--map" && i + 1 < args.Length) g_mapFilter = args[++i];
            }
            ...
            try
            {
                ...
                {
                    ShowMapMenu();
                    mapChoice = GetUserChoice(1, 3);
                    ShowFormatMenu();
                    formatChoice = GetUserChoice(1, 3);
                }

                // Create output directories based on format choice
                string baseDir = outOverride ?? AppDomain.CurrentDomain.BaseDirectory;
        ...
        static string? FindCS2Installation(string steamPath)
        {
            try
            {
                ...
                foreach (string extra in extraInstalls)
                {
                    if (Directory.Exists(Path.Combine(extra, @"game\csgo\maps")))
                        return extra;
                }
        ...
        static void ProcessOfficialMaps(string steamPath, string? triOutputDir, string? vphysOutputDir)
        {
            ...
            var vpkFiles = Directory.GetFiles(officialMapsPath, "*.vpk")
                .Where(f => {
                    ...
                    if (!string.IsNullOrEmpty(g_mapFilter) &&
                        !fileName.Contains(g_mapFilter, StringComparison.OrdinalIgnoreCase))
                        return false;
